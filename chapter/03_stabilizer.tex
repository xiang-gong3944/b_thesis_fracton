...
スタビライザー群の元は一般化パウリ群の元であるから、ある $k\in\mathbb{Z}_p$ と $\mathbf{a},\mathbf{b}\in\mathbb{F}_p^n$ に対して
\begin{equation}
...
% ここまでで、スタビライザー群から符号空間とその上の論理演算子を記述できることを見た。
% 次には、この符号空間を明示的に取り出す方法と、
% それを物理的なハミルトニアンの基底状態空間としてみる方法を考える。
% 特に $\mathbb{Z}_N$ 系では、射影演算子を用いた形式と
% エルミート共役を用いた $\text{h.c.}$ 形式の両方が自然に現れ、
% 両者は同じ基底状態空間を持ちながらも励起スペクトルが異なる。

% まず、1つのスタビライザー演算子 $S$ に対して、
% \begin{equation}
%     \Pi_S := \frac{1}{N}\sum_{m=0}^{N-1} S^m
% \end{equation}
% を考える。
% ここで $S$ の固有状態 $\ket{\psi}$ が
% \begin{equation}
...
% \begin{equation}
%     \Pi_S\ket{\psi}=\ket{\psi}
% \end{equation}
% である。
% 一方、$k\neq 0$ のときには等比級数の和より
% \begin{equation}
%     \sum_{m=0}^{N-1}\omega^{km}=0
% \end{equation}
% であるから、
% \begin{equation}
%     \Pi_S\ket{\psi}=0
% \end{equation}
% となる。
% したがって、$\Pi_S$ はスタビライザー $S$ の $+1$ 固有空間への射影演算子となっている。

% スタビライザー生成元 $S_1,\dots,S_r$ が互いに可換であるならば、
% それぞれの射影演算子も可換である。
% したがって、
% \begin{equation}
%     \Pi_{\mathcal{C}} := \Pi_{S_1}\Pi_{S_2}\cdots \Pi_{S_r}
% \end{equation}
% を考えることができる。
% この演算子は各 $S_i$ の $+1$ 固有空間への共通射影であるから、
% ちょうど符号空間 $\mathcal{C}$ への射影演算子となる。
% このように、スタビライザー形式では符号空間を
% 「全てのスタビライザー条件を満たす状態の集合」とみるだけでなく、
% 射影演算子を用いて明示的に構成することもできる。

% この射影演算子を用いると、符号空間を基底状態空間に持つハミルトニアンを自然に定義できる。
% 最も直接的なものは
% \begin{equation}
%     H_{\mathrm{proj}} := -\sum_{i=1}^r \Pi_{S_i}
% \end{equation}
% である。
% 各 $\Pi_{S_i}$ は固有値 $0$ または $1$ を持つ射影演算子であるから、
% このハミルトニアンのエネルギーが最小になるのは全ての $i$ に対して
% \begin{equation}
%     \Pi_{S_i}\ket{\psi}=\ket{\psi}
% \end{equation}
% が成り立つとき、すなわち全てのスタビライザー条件
% \begin{equation}
%     S_i\ket{\psi}=\ket{\psi}
% \end{equation}
% が満たされるときである。
...
% 一方で、$\mathbb{Z}_N$ 系ではスタビライザー演算子 $S_i$ は一般にエルミートではないため、
% 物理的なハミルトニアンとしては
% \begin{equation}
%     H_{\mathrm{h.c.}} := -\sum_{i=1}^r (S_i+S_i^\dagger)
% \end{equation}
% のような $h.c.$ 形式も自然である。
% $S_i$ の固有値が $\omega^k$ である状態に対しては
% \begin{equation}
%     (S_i+S_i^\dagger)\ket{\psi}
%     = \qty(\omega^k+\omega^{-k})\ket{\psi}
%     = 2\cos\frac{2\pi k}{N}\ket{\psi}
% \end{equation}
% となる。
% したがって、この項のエネルギーが最小になるのは
% \begin{equation}
%     \cos\frac{2\pi k}{N}
% \end{equation}
% が最大となる $k=0$、すなわち $S_i=1$ のときである。
% よって $H_{\mathrm{h.c.}}$ も、$H_{\mathrm{proj}}$ と同じく符号空間 $\mathcal{C}$ を基底状態空間に持つ。

% ただし、これら2つのハミルトニアンは同じ基底状態空間を持っていても、
% 励起スペクトルは異なる。
% 射影形式では、各項の励起エネルギーは
% \begin{equation}
%     \Pi_{S_i}=1 \to 0
% \end{equation}
% という飛び方をするため、$k\neq 0$ の励起は一様に同じだけのエネルギーを持つ。
% それに対して $h.c.$ 形式では、励起エネルギーは
% \begin{equation}
%     -2\cos\frac{2\pi k}{N}
% \end{equation}
% のように $k$ に依存して連続的に変化する。
% 特に $N\to\infty$ 極限では
% \begin{equation}
%     1-\cos\frac{2\pi k}{N}\sim \frac{1}{2}\qty(\frac{2\pi k}{N})^2
% \end{equation}
% となるため、$h.c.$ 形式では励起スペクトルがギャップレスになる。
% 一方、射影形式では $N\to\infty$ としても各励起は有限のエネルギー差を保ち、
% 励起スペクトルはギャップドなままである。
% したがって、射影形式と $h.c.$ 形式は同じ符号空間を記述していても、
% 励起の物理という点では異なる模型を与えている。

% このように、射影形式は符号空間を厳密に取り出すのに適しており、
% $h.c.$ 形式は通常の格子ハミルトニアンとして扱いやすい。
% 特に後に扱う $\mathbb{Z}_N$ toric code では、
% この $h.c.$ 形式が物理的な見通しを与えるうえで重要となる。
% また、ハミルトニアンとして実現を考えるならば、各項が局所相互作用で書けることが重要であり、
% この観点は局所スタビライザーハミルトニアンや量子メモリに求められる条件へとつながっていく。
